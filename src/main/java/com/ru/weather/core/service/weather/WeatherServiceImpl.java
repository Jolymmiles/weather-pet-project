...
public class WeatherServiceImpl implements WeatherService {
    ...
    public List<WeatherEntity> getWeatherByNow(Long id) {
        CityEntity cityEntity = cityEntityRepository.getById(id);
        LocalDate date = LocalDate.now();
        List<WeatherEntity> listOfWeatherNow = new ArrayList<>();
        listOfWeatherNow.add(weatherEntityRepository.findByCityEntityAndDateOfWeather(cityEntity, date));
        if (Objects.nonNull(listOfWeatherNow.get(0))) {
            return listOfWeatherNow;
        } else {
            ...
            listOfWeatherNow.clear();
            listOfWeatherNow.add(weatherEntityRepository.save(weatherToDataBase));
            return listOfWeatherNow;
        }
    }
    ...
    public ResponseEntity<InputStreamResource> getExcelFileOfWeatherWithType(Long id, String type) throws IOException {
        List<WeatherEntity> weatherEntities;
        if (type.equals("today")) {
            logger.info("Получение данных на день");
            weatherEntities = getWeatherByNow(id);
        } else {
            logger.info("Получение данных на неделю");
            weatherEntities = getWeeklyWeather(id);
        }
        ...
        headers.add("Content-Disposition", String.format("attachment; filename=%sWeather.xlsx", type));
